...
| [keycloak-simplified.md](spec/keycloak-simplified.md) | Replaces BC Gov Keycloak/IDIR SSO login with a frontend-driven public-client OIDC flow (Authorization Code + PKCE via `oidc-client-ts`, no client secret). The API only validates the Bearer token and maps `resource_access.<clientId>.roles` to app roles — no cookie relay, no new API auth endpoints. Supersedes [keycloak-integration.md](spec/keycloak-integration.md) (backend-driven token relay, kept for history only). |
