...
| [smb-file-storage.md](spec/smb-file-storage.md) | Moves the **accepted** exhibit store off the pod's disk onto a secure SMB share via the `SMBLibrary` NuGet package; pending uploads stay local. Adds an `SmbFileStorage : IFileStorage` (no new interface methods) plus a session factory, path helper, streaming read `Stream` and write helper. Staged: connect+read diagnostic endpoint → read path → write path → ops. Deliberately far simpler than the `transitory-documents-api` reference because CES authors every path itself and never enumerates the share. |
