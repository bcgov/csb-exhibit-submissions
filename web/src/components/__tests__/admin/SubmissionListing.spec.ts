...
const makeItem = (overrides: Partial<SubmissionReviewModel> = {}): SubmissionReviewModel => ({
  ...
  status: 'Pending',
  exhibitCount: 3,
  tickets: [{ appearanceId: 'A1', fileNumberText: 'FILE001', accusedName: 'Smith, John', appearanceDateTime: '2026-06-01T10:00:00', appearanceSequenceNumber: '001', appearanceReasonCode: 'ADP', courtListType: 'Criminal', accusedDOB: '1980-01-01' }],
  files: [],
  ...overrides,
})
...
describe('SubmissionListing', () => {
  ...
  it('displays exhibit count in the table', async () => {
    mockRetrieveSubmissionListing.mockResolvedValue(makePagedResult([makeItem({ exhibitCount: 5 })]))
    const wrapper = mountListing()
    await flushPromises()
  ...
  it('renders Pending status chip with correct class', async () => {
    mockRetrieveSubmissionListing.mockResolvedValue(makePagedResult([makeItem({ status: 'Pending' })]))
    const wrapper = mountListing()
    await flushPromises()
  ...
  it('renders Accepted status chip with correct class', async () => {
    mockRetrieveSubmissionListing.mockResolvedValue(makePagedResult([makeItem({ status: 'Accepted' })]))
    const wrapper = mountListing()
    await flushPromises()
  ...
  it('renders Rejected status chip with correct class', async () => {
    mockRetrieveSubmissionListing.mockResolvedValue(makePagedResult([makeItem({ status: 'Rejected' })]))
    const wrapper = mountListing()
    await flushPromises()
  ...
  it('shows pagination when totalCount exceeds one page', async () => {
    ...
    mockRetrieveSubmissionListing.mockResolvedValue({ items, totalCount: 45, page: 1, pageSize: 20 })
    const wrapper = mountListing()
    await flushPromises()
